...
"""
If you want to change the bot, permission first!
...
def ___login___():
    os.system('clear')
    print(___logo___)
    print("%s[%s1%s]%s Login Using Token"%(B,P,B,P))
    print("%s[%s2%s]%s Login Using Cookies"%(B,P,B,P))
    print("%s[%s3%s]%s Get Tokens Or Cookies"%(B,P,B,P))
    print("%s[%s4%s]%s Go out"%(K,P,K,P))
    ...
    if ___login___ in ['1','01']:
        ...
        print("%s[%s?%s]%s You Will Be Redirected To Youtube Or Browser"%(B,H,B,P));sleep(2)
        os.system('xdg-open https://youtu.be/XXXXX')
        exit("%s[%s!%s]%s Retype %s«%spython2 dump.py%s»"%(B,K,B,P,H,P,H))
...
def ___follow___():
    ...
    print("%s[%s*%s]%s Login Successful"%(H,P,H,P))
    ___menu___()
# Menu
def ___menu___():
    os.system('clear')
    ...
    print("\n%s[%s1%s]%s Dump Random Mass Public ID  (2004-2021)"%(H,U,H,P))
    print("%s[%s2%s]%s Dump Old Mass Public ID (2009-2006)"%(H,U,H,P))
    print("%s[%s3%s]%s Dump New Mass Public ID (2020-2021)"%(H,U,H,P))
    print("%s[%s4%s]%s Dump Very Old Public ID (2006-2009)"%(H,U,H,P))
    print("%s[%s5%s]%s Dump Old Follower ID (2015-2006)"%(H,U,H,P))
    print("%s[%s6%s]%s Dump New Follower ID (2021-2020)"%(H,U,H,P))
    print("%s[%s7%s]%s Dump Random Public ID (2021-2004)"%(H,U,H,P))
    print("%s[%s8%s]%s Dump Old Public ID (2006-2015)"%(H,U,H,P))
    print("%s[%s9%s]%s Dump New Public ID (2021-2020)"%(H,U,H,P))
    print("%s[%s10%s]%s Start Crack %s[%sFast%s/%sSlow%s]"%(B,U,B,P,K,H,K,H,K))
    print("%s[%s11%s]%s View Crack Results"%(H,U,H,P))
    ...
    if ___menu___ in ['1','01']:
        ...
        print("\n%s[%s1%s]%s View Results Ok.txt"%(B,P,B,P))
        print("%s[%s2%s]%s View Results Cp.txt"%(B,P,B,P))
        ...
        if ___hasilz___ in ['1','01']:
            try:
                ...
                exit("%s[%s!%s]%s Result Ok.txt There is not any"%(P,M,P,M))
            print("%s "%(P))
            os.system('cat Results/Ok.txt')
            print("\n%s[%s*%s]%s Total Results Ok.txt :%s %s"%(B,P,B,P,H,len(open('Results/Ok.txt','r').readlines())))
        elif ___hasilz___ in ['2','02']:
            try:
                ___cp___ = open('Results/Cp.txt','r').read()
            except (IOError):
                exit("%s[%s!%s]%s Results Cp.txt There is not any"%(P,M,P,M))
            print("%s "%(P))
            os.system('cat Results/Cp.txt')
            print("\n%s[%s*%s]%s Total Results Cp.txt :%s %s"%(B,P,B,P,H,len(open('Results/Cp.txt','r').readlines())))
        ...
        os.system("xdg-open https://wa.me/+923011517172?text=Hallo-MANTAL%20Bang%20Rozhak")
        exit()
...
def ___masal___():
    ...
    try:
        ...
        ___total___ = 1
    ___file___ = raw_input("%s[%s?%s]%s Nama File TO SAVE :%s "%(B,P,B,P,H))
    for zx in range(___total___):
        zx +=1
        ___ids___ = raw_input("%s[%s%s%s]%s User :%s "%(B,P,zx,B,P,H))
        print(" ")
        if ___ids___ in ['',' ']:
            exit("%s[%s!%s]%s Don't be Empty"%(P,M,P,M))
        try:
            ...
            print("\r%s                    "%(P))
            print("%s[%s*%s]%s Finished..."%(H,P,H,P))
            print("%s[%s?%s]%s Total ID :%s %s"%(H,P,H,P,K,len(___user___)))
            print("%s[%s?%s]%s File Saved in:%s %s"%(H,P,H,P,K,___file___))
...
def ___masal2___():
    try:
        ...
        exit("%s[%s!%s]%s Token Invalid"%(P,M,P,M))
    try:
        ___total___ = int(raw_input("\n%s[%s?%s]%s Number of ID :%s "%(B,P,B,P,H)))
    except:
        ___total___ = 1
    ___file___ = raw_input("%s[%s?%s]%s Nama File TO SAVE :%s "%(B,P,B,P,H))
    for zx in range(___total___):
